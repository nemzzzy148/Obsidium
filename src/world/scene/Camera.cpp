...
#include "../../../include/world/scene/Camera.h"
...
glm::mat4 Camera::getProjectionMat(const float aspectRatio) const {
    glm::mat4 p(1.0f);
    switch (projection) {
        case Projection::Perspective:
            p = glm::perspective(perspectiveFOV, aspectRatio, perspectiveNear, perspectiveFar);
            break;
        case Projection::Orthographic:
            p = glm::ortho(-orthographicSize * aspectRatio, orthographicSize * aspectRatio,
                -orthographicSize, orthographicSize, orthographicNear, orthographicFar);
            break;
        case Projection::Cavalier:{
            glm::mat4 mat(1.0f);
            mat[2][0] = cosf(glm::radians(angle)) * cavalierSheerX;
            mat[2][1] = sinf(glm::radians(angle)) * cavalierSheerY;
            p = glm::ortho(-orthographicSize * aspectRatio, orthographicSize * aspectRatio,
                -orthographicSize, orthographicSize, orthographicNear, orthographicFar) * mat;
            break;
        }
    }
    return p;
